...
var replayRuntimeContractVersion = "replay-runtime-v1";
...
function assertEffectiveInterval(entry, at, assert) {
  const instant = new Date(at);
  assert(Number.isFinite(instant.getTime()), `Invalid replay instant ${at}`);
  assert(
    new Date(entry.effectiveFrom) <= instant && (entry.effectiveUntil === null || instant < new Date(entry.effectiveUntil)),
    `Version ${entry.kind}:${entry.version} is outside its effective interval at ${at}`
  );
  return entry;
}
function selectEffectiveVersion(entries, kind, at, assert) {
  const candidates = entries.filter(
    (entry) => entry.kind === kind && new Date(entry.effectiveFrom) <= new Date(at) && (entry.effectiveUntil === null || new Date(at) < new Date(entry.effectiveUntil))
  );
...
export {
  assertEffectiveInterval,
  canonicalDigest,
  canonicalizeIJson,
  parseMergeSignalYaml,
  replayRuntimeContractVersion,
