...
function canonicalizeIJson(value) {
  ...
  if (encoded === void 0) throw new TypeError("Value is not canonical I-JSON");
  return encoded;
}
function canonicalDigest(value) {
  return createHash("sha256").update(canonicalizeIJson(value), "utf8").digest("hex");
}
function assertIJsonValue(value, path = "$", seen = /* @__PURE__ */ new Set()) {
...
function parseMergeSignalYaml(bytes) {
  let source;
...
// contracts/version-artifacts/evaluators/feature-evaluator-v1.mjs
...
function samePullRequestIdentity(left, right) {
  ...
  if (!a || !b) return false;
  return a.pullRequestNodeId === b.pullRequestNodeId && a.repositoryNodeId === b.repositoryNodeId && a.pullRequestNumber === b.pullRequestNumber && (a.authorNodeId === null || b.authorNodeId === null || a.authorNodeId === b.authorNodeId);
}
function evaluateReasonPredicate({ predicate, evidenceIds, evidenceById, subjectNodeId, target, features, authoritativeHistoryEvidenceIds }) {
  ...
  const comparisonForTarget = ofType("RELEVANCE_COMPARISON").find(
    (item) => item.canonicalPayload.targetPullRequestNodeId === target.pullRequestNodeId && item.canonicalPayload.targetRepositoryNodeId === target.repositoryNodeId && item.canonicalPayload.targetHeadSha === target.headSha
  )?.canonicalPayload;
  switch (predicate) {
    case "account_tenure_established_v1": {
      const createdAt = ofType("ACCOUNT_CREATED")[0]?.canonicalPayload.createdAt;
      const minimumTenureMs = features.accountTenureMinimumDays * 24 * 60 * 60 * 1e3;
      return Boolean(
        createdAt && ofType("CONTRIBUTION_YEAR").some(
          (item) => item.canonicalPayload.activeMonths.some(
            (month) => /* @__PURE__ */ new Date(`${month}-01T00:00:00Z`) - new Date(createdAt) >= minimumTenureMs
          )
        )
      );
    }
    case "sustained_activity_v1":
      return activeMonths.length >= features.sustainedActivity.minimumActiveMonths && /* @__PURE__ */ new Date(`${activeMonths.at(-1)}-01T00:00:00Z`) - /* @__PURE__ */ new Date(`${activeMonths[0]}-01T00:00:00Z`) >= features.sustainedActivity.minimumSpanDays * 24 * 60 * 60 * 1e3;
    case "multi_year_continuity_v1": {
      ...
      return years.size >= features.multiYearContinuity.minimumActiveYears && activeYears.size >= features.multiYearContinuity.minimumActiveYears && activeMonths.every((month) => years.has(Number(month.slice(0, 4))));
    }
    case "limited_public_history_v1":
      return Boolean(
        coverage && (coverage.completeYears < 2 || coverage.confidence < 0.75 || coverage.partialSources.length > 0 || coverage.attribution !== "complete")
      );
    case "history_partially_accessible_v1":
      return Boolean(coverage && coverage.partialSources.length > 0);
    case "history_truncated_v1":
      return Boolean(
        coverage && coverage.partialSources.some((source) => ["rate_limit", "pagination_limit", "source_limit"].includes(source))
      );
    ...
    case "unsupported_actor_type_v1":
      return ofType("AUTHOR_AVAILABILITY").some((item) => item.canonicalPayload.available) && ofType("ACTOR_TYPE").some((item) => item.canonicalPayload.actorType !== "User") && Boolean(coverage);
    case "author_unavailable_v1":
      return ofType("AUTHOR_AVAILABILITY").some(
    ...
    case "independent_merges_v1": {
      ...
      return relationships.some((relationship) => {
        ...
        return relationship.canonicalPayload.subjectNodeId === subjectNodeId && merged.some(
          (item) => item.canonicalPayload.pullRequestNodeId === pr && item.repositoryNodeId === relationship.repositoryNodeId
        ) && actors.some(
          (item) => item.canonicalPayload.pullRequestNodeId === pr && item.repositoryNodeId === relationship.repositoryNodeId && item.canonicalPayload.githubNodeId !== subjectNodeId
        );
      });
    }
    case "multi_repository_validation_v1":
      return new Set(
        ofType("REPOSITORY_OWNERSHIP_RELATIONSHIP").filter((item) => item.canonicalPayload.classification === "independently_maintained").map((item) => item.repositoryNodeId)
      ).size >= 2;
    case "merge_follow_through_v1": {
      const opened = ofType("PULL_REQUEST_OPENED");
      const merged = ofType("PULL_REQUEST_MERGED");
      return new Set(
        opened.filter((item) => merged.some((candidate) => samePullRequestIdentity(item, candidate))).map((item) => item.canonicalPayload.pullRequestNodeId)
      ).size >= 2;
    }
    case "review_follow_through_v1":
      return ofType("REVIEW_RECEIVED").some(
        (review) => review.canonicalPayload.state === "CHANGES_REQUESTED" && [...ofType("FOLLOW_UP_COMMIT"), ...ofType("REVIEW_THREAD_RESOLVED")].some(
          (followUp) => samePullRequestIdentity(review, followUp) && (followUp.canonicalPayload.commitAuthorNodeId ?? followUp.canonicalPayload.resolverNodeId) === subjectNodeId && new Date(followUp.canonicalPayload.committedAt ?? followUp.canonicalPayload.resolvedAt) > new Date(review.canonicalPayload.submittedAt)
        )
      );
    ...
    case "self_merge_dominated_v1": {
      const closedTypes = /* @__PURE__ */ new Set(["PULL_REQUEST_MERGED", "MERGE_ACTOR", "REPOSITORY_OWNERSHIP_RELATIONSHIP"]);
      ...
      const actors = ofType("MERGE_ACTOR");
      const merged = ofType("PULL_REQUEST_MERGED");
      const classified = relationships.filter(
        (relationship) => merged.some(
          (item) => item.canonicalPayload.pullRequestNodeId === relationship.canonicalPayload.pullRequestNodeId && item.repositoryNodeId === relationship.repositoryNodeId
        )
      );
      const dominated = classified.filter(
        (relationship) => actors.some(
          (actor) => actor.canonicalPayload.pullRequestNodeId === relationship.canonicalPayload.pullRequestNodeId && actor.repositoryNodeId === relationship.repositoryNodeId && actor.canonicalPayload.githubNodeId === subjectNodeId
        )
      );
    ...
    case "patch_inventory_incomplete_v1":
      return ofType("PATCH_FILESET_STATUS").some((item) => !item.canonicalPayload.complete) && ofType("TEST_PATH_CHANGE").some((item) => item.canonicalPayload.state === "unknown") && ofType("SENSITIVE_PATH_CHANGE").some((item) => item.canonicalPayload.state === "unknown");
...
// contracts/version-artifacts/evaluators/assessment-engine-v1.mjs
var assessmentEngineContractVersion = "assessment-engine-v1";
var contextualizationDimensionOrder = Object.freeze([
  "tenure_continuity",
...
function pullRequestIdentity2(item) {
  const payload = item.canonicalPayload;
  if (!payload.pullRequestNodeId) return null;
  return {
    pullRequestNodeId: payload.pullRequestNodeId,
...
function samePullRequestIdentity2(left, right) {
  const a = pullRequestIdentity2(left);
  const b = pullRequestIdentity2(right);
  if (!a || !b) return false;
  return a.pullRequestNodeId === b.pullRequestNodeId && a.repositoryNodeId === b.repositoryNodeId && a.pullRequestNumber === b.pullRequestNumber && (a.authorNodeId === null || b.authorNodeId === null || a.authorNodeId === b.authorNodeId);
}
...
function validateDerivedEvidence({ item, inputs, predicate, features, assert, assertNoDuplicateJsonMembers }) {
  const payload = item.canonicalPayload;
  const ofType = (type) => inputs.filter((input) => input.type === type);
  switch (predicate) {
    case "active_month_v1":
      assert(
        ofType("CONTRIBUTION_YEAR").some(
          (input) => input.canonicalPayload.activeMonths.includes(payload.yearMonth)
        ),
    ...
    case "repository_ownership_v1": {
      ...
      assert(
        [opened, merged, mergeActor].every((input) => samePullRequestIdentity2(opened, input)),
      ...
      const expected = opened.canonicalPayload.repositoryOwnerNodeId === payload.subjectNodeId || opened.canonicalPayload.authorAssociation === "OWNER" || mergeActor.canonicalPayload.githubNodeId === payload.subjectNodeId ? "self_controlled" : ["MEMBER", "COLLABORATOR"].includes(opened.canonicalPayload.authorAssociation) ? "affiliated" : ["CONTRIBUTOR", "FIRST_TIMER", "FIRST_TIME_CONTRIBUTOR", "NONE"].includes(
        opened.canonicalPayload.authorAssociation
      ) && mergeActor.canonicalPayload.githubNodeId !== payload.subjectNodeId ? "independently_maintained" : "unknown";
    ...
    case "template_similarity_v1": {
      ...
      const eligible = ofType("PULL_REQUEST_OPENED").filter(
        (input) => input.canonicalPayload.informativeFeatureCount >= 3 && ownership.some(
          (relationship) => relationship.canonicalPayload.pullRequestNodeId === input.canonicalPayload.pullRequestNodeId && relationship.canonicalPayload.classification === "independently_maintained"
        )
      );
      ...
      const [dominantFingerprint, matchingCount] = [...counts.entries()].sort(
        (left, right) => right[1] - left[1] || compareUtf82(left[0], right[0])
      )[0];
    ...
    case "reciprocal_merge_v1": {
      ...
      const relevant = events.filter((event) => {
        ...
        return value.authorNodeId === payload.subjectNodeId && value.mergeActorNodeId === payload.counterpartyNodeId || value.authorNodeId === payload.counterpartyNodeId && value.mergeActorNodeId === payload.subjectNodeId;
      });
    ...
    case "relevance_comparison_v1": {
      ...
      assert(
        historicalFileset.canonicalPayload.pullRequestNodeId === payload.historicalPullRequestNodeId && historicalFileset.canonicalPayload.repositoryNodeId === payload.historicalRepositoryNodeId && historicalFileset.canonicalPayload.headSha === payload.historicalHeadSha && historicalFileset.canonicalPayload.complete,
        "Relevance historical fileset is not complete and revision-bound"
      );
      assert(
        targetFileset.canonicalPayload.pullRequestNodeId === payload.targetPullRequestNodeId && targetFileset.canonicalPayload.repositoryNodeId === payload.targetRepositoryNodeId && targetFileset.canonicalPayload.headSha === payload.targetHeadSha && targetFileset.canonicalPayload.complete,
        "Relevance target fileset is not complete and head-bound"
      );
      const domainsFor = (repositoryNodeId) => new Set(
        [...ofType("REPOSITORY_TOPIC"), ...ofType("DEPENDENCY_ECOSYSTEM")].filter((input) => input.canonicalPayload.repositoryNodeId === repositoryNodeId).map((input) => (input.canonicalPayload.topic ?? input.canonicalPayload.ecosystem).toLowerCase())
      );
      ...
      const domainMatches = [...historicalDomains].filter((value) => targetDomains.has(value)).sort(compareUtf82);
      ...
      const historicalPaths = ofType("CHANGED_PATH").filter(
        (input) => input.canonicalPayload.pullRequestNodeId === payload.historicalPullRequestNodeId && input.canonicalPayload.repositoryNodeId === payload.historicalRepositoryNodeId && input.canonicalPayload.headSha === payload.historicalHeadSha
      );
      const targetPaths = ofType("CHANGED_PATH").filter(
        (input) => input.canonicalPayload.pullRequestNodeId === payload.targetPullRequestNodeId && input.canonicalPayload.repositoryNodeId === payload.targetRepositoryNodeId && input.canonicalPayload.headSha === payload.targetHeadSha
      );
      assert(
        historicalFileset.canonicalPayload.collectedFileCount === historicalPaths.length && targetFileset.canonicalPayload.collectedFileCount === targetPaths.length,
      ...
      const languageMatches = [...historicalLanguages].filter((value) => targetLanguages.has(value)).sort(compareUtf82);
      assert(
        jsonEquals(payload.languageMatches, languageMatches),
        "Relevance language comparison is not bound to contributed paths"
      );
      const pathMatches = historicalPaths.flatMap(
        (historical) => targetPaths.filter(
          (targetPath) => normalizePathFamily(historical.canonicalPayload.path) === normalizePathFamily(targetPath.canonicalPayload.path)
        ).map((targetPath) => ({
      ...
      ).sort((left, right) => compareUtf82(canonicalizeIJson(left), canonicalizeIJson(right)));
    ...
    case "patch_scope_v1": {
      ...
      assert(paths.every((path) => samePullRequestIdentity2(path, ci)), "Patch scope combines pull requests");
      assert(samePullRequestIdentity2(ci, fileset), "Patch scope fileset targets another pull request");
      ...
      const expected = !payload.filesetComplete ? "unknown" : payload.filesChanged <= 3 && linesChanged <= 20 ? "small" : payload.filesChanged <= 20 && linesChanged <= 500 ? "medium" : "large";
    ...
    case "test_path_change_v1": {
      ...
      const ci = ofType("CI_CHECK_STATE")[0];
      const fileset = ofType("PATCH_FILESET_STATUS")[0];
      assert(ci && fileset && samePullRequestIdentity2(ci, fileset), "Test-path source identity mismatch");
      assert(paths.every((path) => samePullRequestIdentity2(path, ci)), "Test-path inputs cross pull requests");
      ...
      const changed = paths.some(
        (input) => /(^|\/)(?:test|tests|__tests__)(\/|$)|\.(?:test|spec)\.[^/]+$/i.test(input.canonicalPayload.path)
      );
    ...
    case "repository_risk_policy_v1": {
      ...
      const normalizedConfigurationBytes = Buffer.from(canonicalizeIJson(normalizedConfiguration), "utf8");
      const configurationDigest = createHash2("sha256").update(normalizedConfigurationBytes).digest("hex");
      if (payload.configurationSource.kind === "dashboard_revision") {
        ...
        assert(
          payload.configurationSource.revisionEvidenceId === revision.evidenceId && payload.configurationSource.adminPermissionEvidenceId === permission.evidenceId,
          `Dashboard policy ${item.evidenceId} does not bind its exact provenance records`
        );
        assert(
          [revision, permission, policyHead].every(
            (input) => input.canonicalPayload.installationId === payload.installationId && input.canonicalPayload.repositoryNodeId === payload.repositoryNodeId
          ),
          `Dashboard policy ${item.evidenceId} provenance crosses installation or repository scope`
        );
        assert(
          permission.canonicalPayload.permission === "admin" && permission.canonicalPayload.state === "granted" && permission.canonicalPayload.actorGithubNodeId === revision.canonicalPayload.actorGithubNodeId && permission.canonicalPayload.authorizationForRevisionId === revision.canonicalPayload.revisionId && permission.canonicalPayload.authorizationForRevisionSequence === revision.canonicalPayload.revisionSequence && permission.canonicalPayload.authorizationHeadRevision === revision.canonicalPayload.authorizedHeadRevision && revision.canonicalPayload.revisionSequence === revision.canonicalPayload.authorizedHeadRevision && permission.canonicalPayload.authorizationNonce === revision.canonicalPayload.authorizationNonce && permission.canonicalPayload.authorizationSnapshotToken === revision.canonicalPayload.authorizationSnapshotToken && permission.canonicalPayload.providerObservedAt === permission.observedAt,
        ...
        assert(
          permission.observedAt === revision.canonicalPayload.recordedAt && new Date(revision.canonicalPayload.recordedAt) <= new Date(payload.effectiveFrom),
          `Dashboard policy ${item.evidenceId} authorization is not atomically bound to activation`
        );
        assert(
          new Date(payload.effectiveFrom) - new Date(permission.canonicalPayload.providerObservedAt) <= features.dashboardAuthorization.maxAgeSeconds * 1e3,
          `Dashboard policy ${item.evidenceId} uses stale administrator authorization`
        );
        const revisions = ofType("DASHBOARD_POLICY_REVISION").filter(
          (candidate) => candidate.canonicalPayload.installationId === payload.installationId && candidate.canonicalPayload.repositoryNodeId === payload.repositoryNodeId
        ).sort((left, right) => left.canonicalPayload.revisionSequence - right.canonicalPayload.revisionSequence);
        assert(
          revisions.at(-1)?.evidenceId === revision.evidenceId && policyHead.canonicalPayload.highWaterRevision === revision.canonicalPayload.revisionSequence && policyHead.canonicalPayload.highWaterRevisionId === revision.canonicalPayload.revisionId && policyHead.canonicalPayload.streamDigest === createHash2("sha256").update(canonicalizeIJson(revisions.map((candidate) => candidate.canonicalPayload)), "utf8").digest("hex") && policyHead.canonicalPayload.databaseSnapshotToken === revision.canonicalPayload.authorizationSnapshotToken && policyHead.canonicalPayload.serializableReadAt === policyHead.observedAt,
          `Dashboard policy ${item.evidenceId} is not based on an independently observed complete revision stream`
        );
        assert(
          new Date(permission.observedAt) <= new Date(policyHead.observedAt) && new Date(policyHead.observedAt) <= new Date(payload.effectiveFrom) && new Date(payload.effectiveFrom) - new Date(policyHead.observedAt) <= features.dashboardAuthorization.maxAgeSeconds * 1e3,
        ...
        assert(
          payload.configurationSource.defaultBranchEvidenceId === defaultBranch.evidenceId && payload.configurationSource.refSnapshotEvidenceId === refSnapshot.evidenceId && payload.configurationSource.blobSnapshotEvidenceId === blobSnapshot.evidenceId,
          `Default-branch policy ${item.evidenceId} does not bind its exact provider proof chain`
        );
        assert(
          [defaultBranch, refSnapshot, blobSnapshot].every(
            (snapshot) => snapshot.canonicalPayload.installationId === payload.installationId && snapshot.canonicalPayload.repositoryNodeId === payload.repositoryNodeId && snapshot.canonicalPayload.providerObservedAt === snapshot.observedAt
          ),
          `Default-branch policy ${item.evidenceId} proof chain crosses installation or repository scope`
        );
        assert(
          defaultBranch.canonicalPayload.defaultBranchRef === refSnapshot.canonicalPayload.ref && refSnapshot.canonicalPayload.tipCommitSha === blobSnapshot.canonicalPayload.commitSha && blobSnapshot.canonicalPayload.configPath === ".github/mergesignal.yml" && defaultBranch.canonicalPayload.observationBundleId === refSnapshot.canonicalPayload.observationBundleId && refSnapshot.canonicalPayload.observationBundleId === blobSnapshot.canonicalPayload.observationBundleId && defaultBranch.observedAt === refSnapshot.observedAt && refSnapshot.observedAt === blobSnapshot.observedAt,
        ...
        const gitBlobIdentity = createHash2("sha1").update(Buffer.from(`blob ${blobBytes.length}\0`, "utf8")).update(blobBytes).digest("hex");
        ...
        assert(
          canonicalizeIJson(parsedConfiguration) === canonicalizeIJson(normalizedConfiguration),
          `Default-branch policy ${item.evidenceId} YAML does not encode its normalized rules`
        );
        assert(
          blobSnapshot.canonicalPayload.configurationDigest === configurationDigest && canonicalDigest(parsedConfiguration) === configurationDigest,
          `Default-branch policy ${item.evidenceId} configuration digest differs from its blob`
        );
        const latestDefaultBranch = ofType("REPOSITORY_DEFAULT_BRANCH").filter(
          (candidate) => candidate.canonicalPayload.installationId === payload.installationId && candidate.canonicalPayload.repositoryNodeId === payload.repositoryNodeId
        ).sort((left, right) => new Date(right.observedAt) - new Date(left.observedAt) || compareUtf82(left.evidenceId, right.evidenceId))[0];
        ...
        assert(
          new Date(defaultBranch.observedAt) <= new Date(payload.effectiveFrom) && new Date(payload.effectiveFrom) - new Date(defaultBranch.observedAt) <= features.publicHistoryFreshness.maxAgeSeconds * 1e3,
    ...
    case "sensitive_path_change_v1": {
      ...
      assert(ci && fileset && policy && samePullRequestIdentity2(ci, fileset), "Sensitive-path source identity mismatch");
      assert(paths.every((path) => samePullRequestIdentity2(path, ci)), "Sensitive-path inputs cross pull requests");
      ...
      const matches = paths.flatMap(
        (path) => policy.canonicalPayload.rules.filter((rule) => path.canonicalPayload.path.startsWith(rule.pathPrefix)).map((rule) => ({ path: path.canonicalPayload.path, ruleId: rule.ruleId }))
      ).sort((left, right) => compareUtf82(canonicalizeIJson(left), canonicalizeIJson(right)));
      ...
      const state = !fileset.canonicalPayload.complete ? "unknown" : matches.length > 0 ? "changed" : "unchanged";
...
function exactDerivationCandidates(item, derivationIndex) {
  const payload = item.canonicalPayload;
  const typed = (...types) => derivationIndex.typed(item, ...types);
  let candidates;
  switch (item.type) {
    case "ACTIVE_MONTH":
      candidates = typed("CONTRIBUTION_YEAR").filter(
        (candidate) => candidate.canonicalPayload.activeMonths.includes(payload.yearMonth)
      );
      break;
    ...
    case "SENSITIVE_PATH_CHANGE":
      candidates = [
        ...
        ...derivationIndex.repoTyped(item, payload.repositoryNodeId, "REPOSITORY_RISK_POLICY").filter((candidate) => candidate.canonicalPayload.policyId === payload.policyId)
      ];
      break;
    default:
...
function calculateDimension(dimensionName, dimension, coverageConfidence, scoring, manualInspectionReasons) {
  const rule = scoring.dimensions[dimensionName];
  const weightedReasons = dimension.reasonCodes.filter((code) => rule.reasonWeights[code] !== void 0);
  ...
  const confidence = dimension.evidenceIds.length === 0 ? rule.emptyEvidenceConfidence : roundTo(Math.min(
    coverageConfidence,
...
function classifyAssessment({ assessment, assessmentReasonCodes, scoringPolicy, productPolicy, manualInspectionReasons, patchInspectionReasons }) {
  ...
  const qualifiesForEstablishedEvidence = assessment.assessmentStatus === "complete" && assessment.subject.actorType === "User" && assessment.subject.historySupport === "full" && assessment.coverage.completeYears >= 2 && assessment.coverage.confidence >= 0.75 && assessment.overallConfidence.label === "high" && !manualTriggerPresent && assessmentReasons.has("INDEPENDENT_MERGES") && assessmentReasons.has("MULTI_REPOSITORY_VALIDATION") && establishedCoreDimensions.every((dimension) => ["strong", "moderate"].includes(dimension.state));
  ...
  const summaryState = manualTriggerPresent ? "needs_manual_inspection" : qualifiesForEstablishedEvidence ? "established_evidence" : limitingReasonPresent || assessment.assessmentStatus === "partial" || assessment.coverage.completeYears < 2 || assessment.overallConfidence.label === "low" || !supportedDimensionPresent ? "limited_evidence" : "developing_evidence";
  const inspectionTriggerPresent = [...manualInspectionReasons, ...patchInspectionReasons].some((reason) => assessmentReasons.has(reason));
  const reputationPatchRequirements = productPolicy.reviewPriority.reputationAndPatchRequiredFacts;
  const reputationPatchQualified = assessment.patchContext.ciState === reputationPatchRequirements.ciState && reputationPatchRequirements.allowedScopes.includes(assessment.patchContext.scope) && reputationPatchRequirements.allowedTestPathStates.includes(assessment.patchContext.testPathState) && assessment.patchContext.sensitivePathState === reputationPatchRequirements.sensitivePathState && !productPolicy.reviewPriority.forbiddenPatchFacts.some((reason) => assessmentReasons.has(reason));
  const patchOnlyRequirements = productPolicy.reviewPriority.patchOnlyRequiredFacts;
  const patchOnlyFactsQualified = assessment.patchContext.ciState === patchOnlyRequirements.ciState && assessment.patchContext.scope === patchOnlyRequirements.scope && assessment.patchContext.testPathState === patchOnlyRequirements.testPathState && assessment.patchContext.sensitivePathState === patchOnlyRequirements.sensitivePathState && !productPolicy.reviewPriority.forbiddenPatchFacts.some((reason) => assessmentReasons.has(reason));
  const reputationQualifiedForPriority = qualifiesForEstablishedEvidence && assessment.overallConfidence.label === "high" && assessment.coverage.confidence >= 0.75 && productPolicy.reviewPriority.reputationRequiredRelevantExperienceStates.includes(
    assessment.dimensions.relevant_experience.state
  ) && reputationPatchQualified;
  const patchOnlyQualifiedForPriority = productPolicy.reviewPriority.patchQualifiedLimitedHistoryAllowed && ["limited_evidence", "developing_evidence"].includes(assessment.summaryState) && productPolicy.reviewPriority.patchQualifiedActorTypes.includes(assessment.subject.actorType) && patchOnlyFactsQualified;
  const reviewPriority = !assessment.target.riskPolicy.reviewPriorityEnabled ? "not_enabled" : inspectionTriggerPresent ? "inspect_first" : reputationQualifiedForPriority || patchOnlyQualifiedForPriority ? "prioritize" : "standard";
  const reviewPriorityBasis = reviewPriority === "not_enabled" ? "disabled" : reviewPriority === "inspect_first" ? "inspection" : reviewPriority === "standard" ? "standard" : reputationQualifiedForPriority ? "reputation_and_patch" : "patch_only";
  return {
    expectedDimensions,
...
function boundedEvidenceExemplars(sortedPopulation, maximum = 64) {
  ...
  return Array.from(
    { length: maximum },
    (_, index) => sortedPopulation[Math.floor(index * (sortedPopulation.length - 1) / (maximum - 1))]
  );
}
function expandEligibleDerivations(seedIds, items, isEligibleCandidate) {
  const eligible = new Set(seedIds);
  const remainingInputs = /* @__PURE__ */ new Map();
  const dependentsByInput = /* @__PURE__ */ new Map();
...
function buildAuthoritativeHistoryEvidenceIds({
  coverageItem,
  manifestItems,
  assessment,
  evidenceById,
  ...
  const publicHistory = expandEligibleDerivations(
    /* @__PURE__ */ new Set([...coveredCandidateEvidenceIds, coverageItem.evidenceId]),
    manifestItems,
    (item) => item.subjectGithubNodeId === coverageItem.subjectGithubNodeId && item.collectionRunId === coverageItem.collectionRunId && ["PUBLIC_DERIVED", "PUBLIC_GLOBAL"].includes(item.visibility)
  );
  if (!assessment) return publicHistory;
  const eligible = new Set(publicHistory);
  const privateTargetFactTypes = /* @__PURE__ */ new Set([
    "REPOSITORY_LANGUAGE",
    "REPOSITORY_TOPIC",
    "CHANGED_PATH",
    "PATCH_FILESET_STATUS"
  ...
  for (const item of evidenceById.values()) {
    if (item.visibility !== "TARGET_REPOSITORY_PRIVATE" || item.collectionRunId !== coverageItem.collectionRunId || (item.repositoryNodeId ?? item.canonicalPayload.repositoryNodeId) !== assessment.target.repositoryNodeId || !privateTargetFactTypes.has(item.type)) continue;
    const identity = pullRequestIdentity2(item);
    assert(
    ...
    if (item.canonicalPayload.headSha !== void 0) {
      assert(
    ...
    assert(
      new Date(assessment.evidenceSnapshot.capturedAt) - new Date(item.observedAt) <= features.publicHistoryFreshness.maxAgeSeconds * 1e3,
  ...
  return expandEligibleDerivations(
    eligible,
    evidenceById.values(),
    (item) => item.subjectGithubNodeId === assessment.subject.githubNodeId && item.collectionRunId === coverageItem.collectionRunId
  );
}
function evidenceRuleSatisfied(rule, evidenceIds, evidenceById) {
  ...
  return rule.requiredAll.every((type) => types.has(type)) && (rule.requiredAny.length === 0 || rule.requiredAny.some((type) => types.has(type)));
}
function authoritativeReasonEvidenceIds(reason, assessment, evidenceById, authoritativeHistoryEvidenceIds) {
  const allowedTypes = /* @__PURE__ */ new Set([...reason.evidenceRule.requiredAll, ...reason.evidenceRule.requiredAny]);
  const eligibleIds = [...authoritativeHistoryEvidenceIds].filter((id) => allowedTypes.has(evidenceById.get(id)?.type));
  if (reason.dimension !== "relevant_experience") return eligibleIds.sort(compareUtf82);
  const selected = /* @__PURE__ */ new Set();
  for (const id of eligibleIds) {
    const item = evidenceById.get(id);
    if (item.type !== "RELEVANCE_COMPARISON" || item.canonicalPayload.targetPullRequestNodeId !== assessment.target.pullRequestNodeId || item.canonicalPayload.targetRepositoryNodeId !== assessment.target.repositoryNodeId || item.canonicalPayload.targetHeadSha !== assessment.target.headSha) continue;
    selected.add(id);
  ...
  return [...selected].sort(compareUtf82);
}
function boundedPredicateWitness(populationEvidenceIds, reason, evidenceById, evaluate, metrics, assert, maximum = 64) {
  ...
  const relevantTypes = [.../* @__PURE__ */ new Set([...reason.evidenceRule.requiredAll, ...reason.evidenceRule.requiredAny])].sort(compareUtf82);
  const perType = Math.max(1, Math.floor(maximum / Math.max(1, relevantTypes.length)));
  let candidate = [...new Set(relevantTypes.flatMap(
    (type) => boundedEvidenceExemplars(byType.get(type) ?? [], perType)
  ))].sort(compareUtf82).slice(0, maximum);
  const passes = (ids) => {
...
function buildContextualizationCandidates({ assessment, evidenceById, reasonByCode, authoritativeHistoryEvidenceIds, features, assert, metrics = {} }) {
  metrics.populationEvidenceVisits = 0;
  metrics.predicateEvaluations = 0;
  metrics.fullPopulationFallbacks = 0;
  return contextualizationDimensionOrder.flatMap((dimension) => [...assessment.dimensions[dimension].reasonCodes].sort(compareUtf82).map((reasonCode) => {
    ...
    const { witnessMode, witnessEvidenceIds } = boundedPredicateWitness(
      ...
      reason,
      evidenceById,
      (selectedReason, evidenceIds) => evaluateReasonPredicate({
    ...
    return {
      ...
      populationDigest: createHash2("sha256").update(canonicalizeIJson(populationEvidenceIds), "utf8").digest("hex"),
      witnessMode,
      witnessEvidenceIds,
...
function classifyPublication({ fenceState, commentState, checkState, checkConclusion }) {
  return fenceState === "stale" ? "stale" : fenceState === "repair_queued" ? "repair_queued" : checkConclusion === "action_required" ? "action_required" : commentState === "published" && checkConclusion === "success" ? "published" : commentState === "failed" || checkConclusion === "failure" ? "failed" : commentState === "retrying" || checkState === "retrying" ? "retrying" : commentState === "publishing" || checkState === "in_progress" ? "publishing" : "queued";
}
function evaluatePublicationFence({ checkConclusion, postWritePublishable, postCheckPublishable }) {
...
function requiresCommentRemoval({ retentionState, commentId, commentWriteCompletedAt }) {
  return ["subject_deleted", "expired"].includes(retentionState) && commentId !== null && commentWriteCompletedAt !== null;
}
export {
  assessmentEngineContractVersion,
  ...
  classifyAssessment,
  classifyPublication,
  contextualizationDimensionOrder,
